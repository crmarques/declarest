...
simple_api_auth_defaults_for_selected_type() {
  local auth_type=${E2E_MANAGED_SERVICE_AUTH_TYPE:-oauth2}
  ...
  case "${auth_type}" in
    ...
    oauth2)
      ...
      ;;
    custom-header)
      printf 'simple-api-server does not support managed-service auth-type custom-header\n' >&2
      return 1
      ;;
    *)
      printf 'invalid managed-service auth-type for simple-api-server: %s\n' "${auth_type}" >&2
      return 1
      ;;
  esac
}
...
if [[ "${E2E_COMPONENT_CONNECTION}" == 'local' ]]; then
  ...
  simple_api_enable_basic_auth_raw=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_ENABLE_BASIC_AUTH' || true)
  simple_api_enable_oauth2_raw=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_ENABLE_OAUTH2' || true)
  simple_api_enable_mtls_raw=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_ENABLE_MTLS' || true)
  ...
  simple_api_enable_mtls=$(normalize_bool 'DECLAREST_E2E_SIMPLE_API_ENABLE_MTLS' "${simple_api_enable_mtls_raw}" "${E2E_MANAGED_SERVICE_MTLS:-false}") || exit 1
  ...
  simple_api_certs_host_dir=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_CERTS_HOST_DIR' || true)
  simple_api_certs_dir=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_CERTS_DIR' || true)
  : "${simple_api_certs_host_dir:=${E2E_RUN_DIR}/certs/managed-service-simple-api-server}"
  ...
  simple_api_tls_cert_file=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_TLS_CERT_FILE' || true)
  simple_api_tls_key_file=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_TLS_KEY_FILE' || true)
  simple_api_mtls_client_cert_dir=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_MTLS_CLIENT_CERT_DIR' || true)
  simple_api_mtls_client_cert_files=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_MTLS_CLIENT_CERT_FILES' || true)
  ...
  simple_api_tls_ca_cert_file_host=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_TLS_CA_CERT_FILE' || true)
  simple_api_tls_client_cert_file_host=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_TLS_CLIENT_CERT_FILE' || true)
  simple_api_tls_client_key_file_host=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_TLS_CLIENT_KEY_FILE' || true)
  ...
  simple_api_client_id=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_CLIENT_ID' || true)
  simple_api_client_secret=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_CLIENT_SECRET' || true)
  simple_api_basic_auth_username=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_BASIC_AUTH_USERNAME' || true)
  simple_api_basic_auth_password=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_BASIC_AUTH_PASSWORD' || true)
  simple_api_scope=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_SCOPE' || true)
  simple_api_audience=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_AUDIENCE' || true)
  ...
  e2e_write_state_value "${state_file}" MANAGED_SERVICE_ACCESS_BASE_URL "${simple_api_base_url}"
  e2e_write_state_value "${state_file}" MANAGED_SERVICE_BASE_URL "${simple_api_base_url}"
  ...
  e2e_write_state_value "${state_file}" MANAGED_SERVICE_AUTH_KIND "$(simple_api_resolved_auth_kind "${simple_api_enable_basic_auth}" "${simple_api_enable_oauth2}")"
  e2e_write_state_value "${state_file}" MANAGED_SERVICE_ACCESS_AUTH_MODE "${E2E_MANAGED_SERVICE_AUTH_TYPE:-oauth2}"

  if [[ "${simple_api_enable_oauth2}" == 'true' ]]; then
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_TOKEN_URL "${simple_api_token_url}"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_OAUTH_CLIENT_ID "${simple_api_client_id}"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_OAUTH_CLIENT_SECRET "${simple_api_client_secret}"
  elif [[ "${simple_api_enable_basic_auth}" == 'true' ]]; then
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_BASIC_USERNAME "${simple_api_basic_auth_username}"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_BASIC_PASSWORD "${simple_api_basic_auth_password}"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_ACCESS_USERNAME "${simple_api_basic_auth_username}"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_ACCESS_PASSWORD "${simple_api_basic_auth_password}"
  else
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_HEADER_NAME "Authorization"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_HEADER_PREFIX "Bearer"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_HEADER_VALUE "simple-api-oauth2-disabled"
  fi
  ...
  if [[ -n "${simple_api_scope}" ]]; then
    ...
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_OAUTH_SCOPE "${simple_api_scope}"
  fi
  if [[ -n "${simple_api_audience}" ]]; then
    e2e_write_state_value "${state_file}" SIMPLE_API_SERVER_AUDIENCE "${simple_api_audience}"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_OAUTH_AUDIENCE "${simple_api_audience}"
  fi
  if [[ "${simple_api_enable_mtls}" == 'true' ]]; then
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_TLS_ENABLED "true"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_TLS_CA_CERT_FILE_HOST "${simple_api_tls_ca_cert_file_host}"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_TLS_CLIENT_CERT_FILE_HOST "${simple_api_tls_client_cert_file_host}"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_TLS_CLIENT_KEY_FILE_HOST "${simple_api_tls_client_key_file_host}"
    e2e_write_state_value "${state_file}" MANAGED_SERVICE_TLS_INSECURE_SKIP_VERIFY_FOR_CLUSTER "true"
  fi
  exit 0
fi

simple_api_base_url=$(e2e_require_env 'DECLAREST_E2E_MANAGED_SERVICE_BASE_URL') || exit 1
simple_api_auth_defaults=$(simple_api_auth_defaults_for_selected_type) || exit 1
read -r simple_api_default_basic_auth simple_api_default_oauth2 <<<"${simple_api_auth_defaults}"
simple_api_enable_basic_auth_raw=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_ENABLE_BASIC_AUTH' || true)
simple_api_enable_oauth2_raw=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_ENABLE_OAUTH2' || true)
...
simple_api_token_url=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_TOKEN_URL' || true)
simple_api_scope=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_SCOPE' || true)
simple_api_audience=$(e2e_env_optional 'DECLAREST_E2E_SIMPLE_API_AUDIENCE' || true)
...
if [[ "${simple_api_enable_oauth2}" == 'true' ]]; then
  simple_api_client_id=$(e2e_require_env 'DECLAREST_E2E_SIMPLE_API_CLIENT_ID') || exit 1
  simple_api_client_secret=$(e2e_require_env 'DECLAREST_E2E_SIMPLE_API_CLIENT_SECRET') || exit 1
fi
if [[ "${simple_api_enable_basic_auth}" == 'true' ]]; then
  simple_api_basic_auth_username=$(e2e_require_env 'DECLAREST_E2E_SIMPLE_API_BASIC_AUTH_USERNAME') || exit 1
  simple_api_basic_auth_password=$(e2e_require_env 'DECLAREST_E2E_SIMPLE_API_BASIC_AUTH_PASSWORD') || exit 1
fi
...
if [[ "${simple_api_enable_mtls}" == 'true' ]]; then
  simple_api_tls_ca_cert_file_host=$(e2e_require_env 'DECLAREST_E2E_SIMPLE_API_TLS_CA_CERT_FILE') || exit 1
  simple_api_tls_client_cert_file_host=$(e2e_require_env 'DECLAREST_E2E_SIMPLE_API_TLS_CLIENT_CERT_FILE') || exit 1
  simple_api_tls_client_key_file_host=$(e2e_require_env 'DECLAREST_E2E_SIMPLE_API_TLS_CLIENT_KEY_FILE') || exit 1
fi
