...
case_run() {
  ...
  case_run_declarest resource get "${secret_path}" --source managed-service -o json
  case_expect_success
  if ! jq -e \
    --arg name "${secret_name}" \
    '.name == $name and .type == "password" and .contentType == "application/x-rundeck-data-password"' \
    <<<"${CASE_LAST_STDOUT}" >/dev/null; then
    printf 'expected Rundeck managed-service output to describe the nested password secret\n' >&2
    printf 'output: %s\n' "${CASE_LAST_OUTPUT}" >&2
    return 1
  fi

  case_run_declarest resource list "${project_path}/secrets/${nested_dir}" --source managed-service -o json
  case_expect_success
  if ! jq -e \
    --arg name "${secret_name}" \
