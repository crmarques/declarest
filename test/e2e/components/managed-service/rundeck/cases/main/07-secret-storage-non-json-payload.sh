...
CASE_SCOPE='main'
CASE_REQUIRES='managed-service=rundeck has-secret-provider'
...
EOF
  ...
  case_run_declarest resource get "${public_key_path}" --source managed-service -o json
  case_expect_success
  if ! jq -e \
    --arg name "${secret_name}" \
    '.name == $name and .type == "public" and .contentType == "application/pgp-keys"' \
    <<<"${CASE_LAST_STDOUT}" >/dev/null; then
    printf 'expected Rundeck managed-service metadata to describe the saved armored key\n' >&2
    printf 'output: %s\n' "${CASE_LAST_OUTPUT}" >&2
    return 1
  fi

  case_run_declarest resource list "${project_path}/infra-secrets" --source managed-service -o json
  case_expect_success
  if ! jq -e \
    --arg name "${secret_name}" \
  ...
  case_run_declarest resource list "${project_path}/secrets" --source managed-service -o json
  case_expect_success
  if ! jq -e \
    --arg name "${secret_name}" \
...
EOF
  ...
  case_run_declarest resource get "${private_key_path}" --source managed-service -o json
  case_expect_success
  if ! jq -e '. == "{{secret .}}"' <<<"${CASE_LAST_STDOUT}" >/dev/null; then
    printf 'expected managed-service output to redact the whole-resource private key\n' >&2
    printf 'output: %s\n' "${CASE_LAST_OUTPUT}" >&2
    return 1
  fi
}
