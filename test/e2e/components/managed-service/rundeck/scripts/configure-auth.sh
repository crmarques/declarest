...
case "${selected_auth_type}" in
  basic|prompt)
    ...
    e2e_write_state_value "${E2E_COMPONENT_STATE_FILE}" MANAGED_SERVICE_BASIC_USERNAME "${RUNDECK_ADMIN_USER}"
    e2e_write_state_value "${E2E_COMPONENT_STATE_FILE}" MANAGED_SERVICE_BASIC_PASSWORD "${RUNDECK_ADMIN_PASSWORD}"
...
if [[ -n "${token}" ]]; then
  ...
  e2e_write_state_value "${E2E_COMPONENT_STATE_FILE}" MANAGED_SERVICE_AUTH_KIND "custom-header"
  e2e_write_state_value "${E2E_COMPONENT_STATE_FILE}" MANAGED_SERVICE_HEADER_NAME "${auth_header}"
  e2e_write_state_value "${E2E_COMPONENT_STATE_FILE}" MANAGED_SERVICE_HEADER_VALUE "${token}"
  e2e_write_state_value "${E2E_COMPONENT_STATE_FILE}" MANAGED_SERVICE_ACCESS_AUTH_MODE "custom-header"
  e2e_write_state_value "${E2E_COMPONENT_STATE_FILE}" MANAGED_SERVICE_ACCESS_HEADER "${auth_header}"
  e2e_write_state_value "${E2E_COMPONENT_STATE_FILE}" MANAGED_SERVICE_ACCESS_TOKEN "${token}"
  exit 0
fi
