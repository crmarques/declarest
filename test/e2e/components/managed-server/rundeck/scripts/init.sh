...
if [[ "${selected_auth_type}" != 'custom-header' ]]; then
  e2e_die "managed-server rundeck remote connection supports only auth-type custom-header"
  exit 1
fi

rundeck_base_url=$(e2e_require_env 'DECLAREST_E2E_MANAGED_SERVER_BASE_URL' 'E2E_MANAGED_SERVER_BASE_URL') || exit 1
rundeck_token=$(e2e_require_env 'DECLAREST_E2E_MANAGED_SERVER_TOKEN' 'E2E_MANAGED_SERVER_TOKEN') || exit 1
rundeck_api_version=$(e2e_env_optional 'DECLAREST_E2E_MANAGED_SERVER_RUNDECK_API_VERSION' 'E2E_MANAGED_SERVER_RUNDECK_API_VERSION' || true)
rundeck_api_version=${rundeck_api_version:-45}
rundeck_auth_header=$(e2e_env_optional 'DECLAREST_E2E_MANAGED_SERVER_RUNDECK_AUTH_HEADER' 'E2E_MANAGED_SERVER_RUNDECK_AUTH_HEADER' || true)
