...
EOF
  ...
  case_run_declarest resource get "${project_path}" --source managed-server --prune-defaults -o json
  case_expect_success
  case_expect_output_contains '"displayName": "Defaults Sandbox"'
  case_expect_output_not_contains '"owner": "defaults-team"'
  ...
  case_run_declarest resource list /api/projects --source managed-server -o json
  case_expect_success
