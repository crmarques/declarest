...
if [[ "${selected_auth_type}" != 'custom-header' ]]; then
  e2e_die "managed-server vault does not support auth-type ${selected_auth_type} (supported: custom-header)"
  exit 1
fi
...
if [[ "${E2E_COMPONENT_CONNECTION}" == 'local' ]]; then
  ...
  e2e_write_state_value "${state_file}" MANAGED_SERVER_BASE_URL "${vault_address}"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_AUTH_KIND "custom-header"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_HEADER_NAME "X-Vault-Token"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_HEADER_VALUE "${vault_token}"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_BASE_URL "${vault_address}"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_AUTH_MODE "custom-header"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_HEADER "X-Vault-Token"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_TOKEN "${vault_token}"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_MOUNT "secret"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_PATH_PREFIX "declarest-e2e"
  e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_KV_VERSION "2"
  exit 0
fi
...
e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_HEADER "X-Vault-Token"
e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_TOKEN "${vault_token}"
e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_MOUNT "${vault_mount}"
e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_PATH_PREFIX "${vault_path_prefix}"
e2e_write_state_value "${state_file}" MANAGED_SERVER_ACCESS_KV_VERSION "${vault_kv_version}"
