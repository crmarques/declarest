...
`--metadata-source bundle` uses the selected managed-service component `METADATA_BUNDLE_REF` when declared, seeds the local `~/.declarest/metadata-bundles/<name>-<version>/` cache by copying `bundle.yaml`, `metadata/`, and `openapi.yaml` from the peer monorepo `${E2E_METADATA_BUNDLES_ROOT}/bundles/<component>/` (default `../declarest-metadata-bundles`), and skips local `openapi.yaml` wiring so `managedService.http.openapi` stays unset. When a component has no bundle ref but a peer bundle directory exists, its `metadata/` tree is reused for the run workspace copy.
`--metadata-source dir` prefers the peer bundles-repo `metadata/` tree (when present) as the `metadata.baseDir` source and otherwise falls back to the component-local `metadata/` dir, keeping normal local `openapi.yaml` wiring (resolved from the peer bundle when available).
...
- `DECLAREST_E2E_LOCAL_ACCESS_HOST`: optional non-loopback host override used when compose local-proxy runs must rewrite loopback component URLs for host-side CLI access
