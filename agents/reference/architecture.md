...
3. `orchestrator.Orchestrator` MUST be the only orchestrator that coordinates repository, metadata, server, and secrets in one workflow.
4. Public/domain packages MUST depend on interfaces, never provider concrete types.
5. `internal/cli` MUST orchestrate command behavior through orchestrator/context interfaces, not providers.
...
3. Public domain packages: `config`, `resource`, `metadata`, `repository`, `server`, `secrets`, `orchestrator`.
...
2. `internal/cli/*` -> `internal/cli/common`, `config`, `orchestrator`.
3. `orchestrator` -> `repository`, `metadata`, `server`, `secrets`, `resource`.
...
2. Domain packages (`resource`, `metadata`, `orchestrator`, `core`) importing `cmd` or `internal/cli`.
...
## Examples
1. Correct: orchestrator depends on `server.ResourceServerManager` interface and receives HTTP provider through `core`.
