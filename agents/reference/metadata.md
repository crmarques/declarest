...
19. `resource.id: "{{/id}}"`, `resource.alias: "{{/clientId}}"`, and `resource.requiredAttributes: ["/realm"]` allow a structured create payload with `/realm` and `/clientId` only when the server assigns `/id`, while update for that same scope still requires `/id` unless operation validation already declares a different source.
