...
4. `core.ExecutionRuntime`: active runtime managers and configuration identity.
...
2. Metadata on `/customers/_` sets default `get.path`; resource metadata on `/customers/acme` overrides only `update.path`.
