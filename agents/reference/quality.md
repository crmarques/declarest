...
55. Plain-text-only CLI commands (`secret get`, `repo tree`, `managed-server get *`, `managed-server check`, shell `completion` subcommands, and `config print-template`) reject `--output json|yaml` with `ValidationError` instead of silently ignoring structured output requests.
