...
30. CLI path completion: completion merges repository paths, remote paths, and OpenAPI paths with command-aware source priority (remote-first defaults for `resource get|save|list|delete`, repository-first defaults for `resource apply|create|update|diff|explain|template` with fallback when the preferred source has no candidates); templated OpenAPI segments resolve concrete values through source-prioritized collection listings, placeholder segments (`{...}`) are excluded from rendered suggestions, alias-aware segments prefer metadata `aliasFromAttribute` values over IDs when available, scoped collection-prefix suggestions render deterministic next-fragment outputs (for example `/alpha`), top-level collection-prefix suggestions preserve trailing `/`, and path completions emit `NoSpace` so accepted completions do not auto-append a trailing space.
...
5. CLI test verifies `metadata infer --apply` persists the same compact payload printed to output and that `metadata get` fallback behavior is gated by OpenAPI/remote endpoint existence.
