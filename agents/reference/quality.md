...
18. OLM packaging tests verify `kustomize build config/manifests`, `kustomize build config/olm`, `operator-sdk bundle validate --select-optional suite=operatorhub ./bundle`, and `opm validate ./catalog` all succeed; the bundled CSV lists every `declarest.io/v1alpha1` owned CRD with `alm-examples`, declares webhook definitions for every admission webhook, and the bundled Deployment does not carry an OLM-incompatible `PersistentVolumeClaim`.
