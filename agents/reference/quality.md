...
86. E2E runner selection flag contract: runner selection flags use canonical namespaces for managed-server and shared proxy behavior (`--managed-server`, `--managed-server-connection`, `--managed-server-auth-type`, `--managed-server-mtls`, `--proxy-mode`, `--proxy-auth-type`); `--managed-server-auth-type prompt` emits `managedServer.http.auth.prompt` only for basic-auth-capable components; deprecated `--managed-server-proxy*` aliases remain accepted but conflicting canonical-vs-legacy values fail validation.
