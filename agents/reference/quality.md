...
32. Resource defaults CLI contract: `resource defaults get|edit|infer` command-level coverage MUST verify raw defaults reads, empty-object behavior when the sidecar is absent, editor persistence/clear behavior, `--save` persistence, and the `--managed-server` safety gate requiring `--yes`.
