...
36. Context validation contract: all context-catalog mutation and resolve flows fail with `ValidationError` when `resource-server` is missing, `resource-server.http.proxy` rejects missing proxy URLs or incomplete auth credentials, interactive `config add` always prompts resource-server configuration, and `config edit` rejects invalid edited catalog/context YAML without persisting partial changes.
...
69. E2E managed-server proxy contract: `--managed-server-proxy` toggles context proxy injection (`resource-server.http.proxy`) and fails validation when enabled without at least one configured proxy URL (`DECLAREST_E2E_MANAGED_SERVER_PROXY_HTTP_URL` or `DECLAREST_E2E_MANAGED_SERVER_PROXY_HTTPS_URL`).
