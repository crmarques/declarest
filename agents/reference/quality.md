...
18. Ad-hoc CLI routing: `ad-hoc <method>` maps to resource-server requests with positional/flag path validation and payload decoding from `--file <path|->` or stdin, and `ad-hoc post|put --payload` inline decoding with source-conflict validation.
