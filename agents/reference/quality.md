...
11. OLM packaging changes (`config/manifests/`, `config/olm/`, `bundle/`, `catalog/`, `bundle.Dockerfile`, `catalog.Dockerfile`, release workflows publishing bundle/catalog images) MUST pass `make verify-bundle` (kustomize build of `config/manifests` and `config/olm`, `operator-sdk bundle validate --select-optional suite=operatorframework ./bundle`, `opm validate ./catalog`, version/image consistency checks, and before/after artifact drift detection) before handoff.
12. Workflows that publish release artifacts or container images MUST pin external actions to full commit SHAs, grant the least `GITHUB_TOKEN` permissions needed per job, and publish provenance attestations for released CLI artifacts and container images.
13. Published container-image workflows MUST request BuildKit provenance and SBOM attestations for pushed images.
...
### Core Semantics
1. Metadata precedence: shared-source vs repo-local overlay precedence, wildcard/literal collisions inside one source, and template resolution (including relative references).
...
5. Compare/diff semantics: suppression/filter rules, stable ordering, structured diff/explain `resource.DiffEntry` path shape (`ResourcePath` + RFC 6901 `Path` pointer with `Path=""` for root replace), normalized unified text rendering for `resource diff --output auto|text`, stable grouped ordering for multi-resource diff sections, stable `resource diff --list` path-only output, and `--color <auto|always|never>` behavior.

### CLI Behavior
6. CLI execution footer and warnings: resource mutation commands (`resource save|apply|create|update|delete`) and state-changing HTTP request commands (`resource request post|put|patch|delete|connect`) emit deterministic `[OK|ERROR] ...` status lines to stderr unless `--skip-result-message` is set, standalone warning lines emit deterministic `[WARNING] ...` labels unless `--ignore-warnings` suppresses them, interactive terminals apply bold green/red/yellow color tags respectively, `--no-color`/`NO_COLOR` disable ANSI color tags, nil payload outputs stay empty, state-changing commands suppress payload output by default, and `--verbose` restores that payload output.
7. CLI safeguards: validation errors, conflicting path inputs, and destructive-operation protections.
8. Metadata edit CLI: `resource metadata edit` opens YAML metadata content, seeds empty metadata for missing overrides, rejects invalid edited YAML, and persists only validated metadata updates.
9. Global flag env defaults: `DECLAREST_CONTEXT`, `DECLAREST_OUTPUT`, `DECLAREST_VERBOSE`, `DECLAREST_VERBOSE_INSECURE`, `DECLAREST_SKIP_RESULT_MESSAGE`, legacy `DECLAREST_NO_STATUS`, `DECLAREST_IGNORE_WARNINGS`, `DECLAREST_NO_COLOR`, and `NO_COLOR` honor `flag > env > built-in default` precedence, and explicit `--skip-result-message=false`, `--ignore-warnings=false`, or `--no-color=false` override env-backed defaults.

### Context and Config
9. Context config: strict decode with no legacy aliases, one-of validation (including repository-only and remote-only contexts), top-level `credentials` plus `credentialsRef` validation/injection, metadata-source one-of behavior, shared-source plus repo-local overlay wiring, overrides precedence, and missing-catalog behavior.
10. Context defaults: omitted `repository.git.remote.autoSync` enables automatic push behavior, and omitted `managedService.http.healthCheck` probes the normalized `managedService.http.url`.
...
10. Metadata bundles: strict `bundle.yaml` decode (required fields, semver `version`, illegal nested keys rejected, `distribution.artifactTemplate` naming contract), shorthand name/version contract checks, secure tar.gz extraction safeguards, deterministic cache reuse behavior, `declarest.compatibleDeclarest` constraint enforcement against the running binary version (with `dev` build bypass), and `declarest.compatibleManagedService.{product,versions}` decode-time syntactic validation.
11. Bundle OpenAPI wiring: `managedService.http.openapi` precedence over bundle hints, fallback to `bundle.yaml declarest.openapi`, bundle-root/metadata-root OpenAPI files (`openapi.yaml|openapi.yml|openapi.json`), recursive bundle OpenAPI file discovery, and cross-origin OpenAPI fetches without leaked auth headers.
12. Managed-service OpenAPI version compatibility: `openapi: 3.x` and `swagger: 2.0` specs provide equivalent method-support checks, media fallback defaults, and `validate.schemaRef=openapi:request-body` behavior for body-bearing operations.
13. Remote operation construction: OpenAPI-assisted defaults with explicit metadata override precedence, including inferred metadata `resource.format` and `format: any` when mixed payload formats are advertised.
...
15. Metadata-native defaults semantics: `resource.defaults` mode/profile resolution, deterministic include-backed defaults artifacts (`defaults.<ext>`, `defaults-<profile>.<ext>`), supported defaults codecs (`json|yaml|yml|properties`), raw metadata persistence vs resolved metadata expansion, unsupported payload rejection (`xml|hcl|text|octet-stream|opaque suffixes`), preservation of explicit empty override files for fully-defaulted resources, deterministic behavior when only metadata defaults exist without a child resource override, and managed-service probe-create payload minimization from create-required attributes with implicit alias-pointer inclusion.

### E2E Harness
15. E2E profiles: `cli-basic|cli-full|cli-manual|operator-manual|operator-basic|operator-full` workload behavior, `smoke|main|operator-main|corner` case selection, `CASE_PROFILES` family filtering, deterministic step statuses, manual-profile managed-service access handoff details, and operator-profile YAML-driven OLM installation/handoff/automation validation.
16. E2E runtime UX: grouped step reporting (`RUNNING|OK|FAIL|SKIP`), live `SPAN` elapsed-time updates for running steps, actionable failure log pointers, and final-summary execution parameters that distinguish explicit selections from defaulted values (including profile defaults, component-elected defaults, env-backed container engine selection, and proxy auth-type resolution from explicit flags versus inline proxy auth env vars).
...
19. E2E parallel launcher: `run-e2e-parallel.sh` command-matrix parsing, per-job log emission, aggregated non-zero exit status when any child run fails, and run-scoped host-port reservation/release for concurrent local stacks.

### Authentication
19. OAuth2 component auth: `client_credentials` token issuance, managed-service auth rejection when oauth2 config is missing or invalid, and bootstrap warnings when `managedService.http.auth.oauth2.tokenURL` uses plain HTTP.
...
22. Prompt-auth component auth: prompt-backed managed-service, git, vault, and proxy flows prompt once per command when uncached, fail without an interactive terminal when no cached session credentials are available, ask for explicit reuse confirmation before sharing credentials with other prompt-auth components in the same command, persist cross-command prompt-auth caches only under `XDG_RUNTIME_DIR/declarest/prompt-auth/`, reuse kept component credentials across later commands only inside shell sessions that evaluated `context session-hook bash|zsh`, stop reusing them after `context clean --credentials-in-session` or shell exit, and fall back to current-process-only reuse when `XDG_RUNTIME_DIR` is unavailable.
23. Managed-service auth redaction: debug logging masks `Authorization` and custom auth header names defined by `managedService.http.auth.customHeaders` unless `--verbose-insecure` is enabled, while non-auth headers remain visible.

### HTTP Request CLI
23. HTTP request CLI routing: `resource request <method>` maps to managed-service requests with positional/flag path validation, metadata-rendered header propagation, payload decoding from `--payload <path|->` or stdin, inline decoding only for structured payload formats, `application/octet-stream` for opaque payloads, and `--accept-type` / `--content-type` overrides for raw non-metadata endpoints.
...
### Resource Operations
27. Resource collection-target scope: `resource apply|create|update|delete|diff` execute direct-child collection targets by default; `apply|create|update|delete|diff` include descendants with `--recursive` when supported; `resource diff` renders grouped text sections only for changed/added/removed resources, `resource diff --list` returns only stable changed paths, and structured `resource diff` output preserves the deterministic flattened `resource.DiffEntry` list; `resource apply|create|update` accept explicit payload input for single-target mutations while repository-driven mode loads payloads for resolved targets when explicit input is absent; explicit-payload mutations MUST include CLI coverage for collection-path child inference from metadata alias/id plus the equivalent explicit resource path.
...
30. Resource save list identity fallback: when list entries omit metadata-defined alias/id attributes, `resource save` falls back to common identity attributes (`clientId`, `id`, `name`, `alias`) before returning alias-resolution errors, preserves per-entry payload descriptors when they are available, and `resource.format: any` prevents one collection-level descriptor from coercing every saved child to the same repository format.
31. Metadata defaults composition contract: repository-backed `resource get|apply|create|update|diff|template|edit|copy|save` use the effective payload merged from resolved metadata defaults and raw `resource.<ext>` for merge-capable object resources, while repository writes compact back into `resource.<ext>` so unchanged defaulted fields remain only in metadata-managed defaults artifacts or inline metadata values.
32. Resource defaults CLI contract: `resource defaults get|edit|config get|config edit|profile get|profile edit|profile delete|infer` command-level coverage MUST verify raw metadata persistence vs resolved-defaults output, empty-object behavior when defaults are absent, editor persistence/clear behavior, deterministic defaults artifact file selection, collection-path vs concrete-resource-path inference inputs (including equivalent collection paths with and without trailing `/`), `--from` parsing with default `repository`, `--items` alias filtering plus missing-alias failures, `--save` persistence including defaults-artifact codec selection plus writable-target selection for explicit `metadata.baseDir` versus `metadata.bundle|bundleFile` and stdout suppression on success, `--check` success/mismatch behavior, the managed-service confirmation gate (`--yes` and interactive prompt path), `--wait` validation plus parsing of bare-integer seconds vs explicit durations, and managed-service probe generation from raw selected local items without pulling values from resolved metadata defaults sidecars.
33. Defaults pruning contract: `resource get --prune-defaults` MUST compact repository and managed-service outputs against resolved metadata defaults while preserving empty-object output when all fields are defaulted and pruning fields covered by stable empty-object defaults, and `resource save --prune-defaults` MUST compact single-resource and per-item list saves before persistence while keeping explicit empty override files for fully-defaulted saved resources.

### Read Fallback
32. Metadata-aware read fallback: `resource get` and `resource save` (no input) attempt literal remote read first and then collection list/filter fallback by metadata alias/id when `NotFound`; `resource request get` mirrors this behavior after a `NotFound` literal request; and managed-service read fallback MUST also cover complex alias templates whose `operations.get.path` cannot be rendered from the requested logical segment alone by resolving one unique matched parent-collection candidate payload.
...
43. Context add interactive schema coverage: wizard prompts require managed-service capture, allow optional-section skipping for optional blocks, support full context attribute capture across repository/managed-service/secret-store/preference blocks, and enforce one-of prompt branching so only the selected auth/key-source/provider branch is collected.
...
45. Context validation contract: context-catalog mutation and resolve flows fail with `ValidationError` when both `repository` and `managedService` are missing, preserve repository-only and remote-only contexts as valid shapes, reject legacy alias keys during strict decode, allow sparse proxy override blocks while still rejecting invalid proxy URLs or incomplete `credentialsRef` blocks, resolve referenced credentials into git/managed-service/vault/proxy basic-auth consumers, interactive `context add` still prompts managed-service configuration, and `context edit` rejects invalid edited catalog/context YAML without persisting partial changes.
...
### Metadata CLI
49. Metadata selector-path contract: `resource metadata infer|render|get` accept intermediary selector paths (for example `/admin/realms/_/clients/`), `resource metadata render` defaults operation by target kind and retries `list` when defaulted `get` is missing a path, infer uses OpenAPI hints when available (including fallback placeholder normalization for non-template-safe OpenAPI parameter names), `resource metadata infer` exposes only supported flags (no hidden recursive mode), structured metadata output omits nil directive fields, and infer output omits directives equal to deterministic fallback defaults.

### Secret Output
50. Resource get secret-output contract: `resource get` redacts metadata-declared secret attributes to `{{secret .}}` for `--source repository` and `--source managed-service` by default, and `--show-secrets` restores plaintext output for those attributes.
...
52. Collection exclude contract: `resource get|list|save --exclude <item[,item...]>` exclude matching collection items by alias, ID, or direct child path segment for managed-service and repository collection reads, and `resource save --exclude` rejects `--mode single` with `ValidationError`.
...
54. Metadata get output contract: `resource metadata get` returns the full canonical nested metadata schema by default with explicit default values for unset attributes, `resource metadata get --overrides-only` returns the compact override object without expanded defaults, fallback inference remains endpoint-gated (OpenAPI or remote discovery), and commands still return `NotFoundError` when neither source resolves the endpoint.
55. Metadata infer apply persistence contract: `resource metadata infer --apply` persists compacted metadata only (no default-equivalent directives), matching the command output contract, and JSON output/persisted JSON metadata end with one trailing newline.
...
- Descendant-scoped metadata contract: non-root collection metadata does not inherit into deeper descendants unless `selector.descendants: true` is set, descendant-enabled renders expose `descendantPath` and `descendantCollectionPath` without mutating payload-validation input, nested collection lists resolve relative to the matched descendant collection root, and Rundeck-style nested secret paths (for example `/projects/<project>/secrets/path/to/db-password`) render `/storage/keys/project/<project>/path/to/db-password` for `get|create|update|delete|compare` plus `/storage/keys/project/<project>/path/to` for nested `list`.
...
65. Completion cursor-scope contract: shell completion scripts MUST truncate completion requests to the active cursor position so positional path completion still works when later arguments remain on the command line.

### Server CLI
66. Resource-server utility CLI contract: `server get base-url|token-url|access-token` print plaintext values from the active managed-service configuration/auth flow, `get token-url|access-token` fail with `ValidationError` when OAuth2 auth is not configured, and `server check` performs a GET probe against `managedService.http.healthCheck` when configured (otherwise the normalized `managedService.http.url` path) and succeeds only on successful HTTP responses.

### Payload Type
67. Payload-type directive and media-default contract: metadata template rendering supports canonical pointer placeholders such as `{{/id}}`, one-level shorthand such as `{{id}}`, compatibility legacy `{{.id}}`, plus `{{payload_type .}}`, `{{payload_media_type .}}`, and `{{payload_extension .}}`; template scopes inject `contentType` from the active payload descriptor when the payload omits it, `resource metadata get` preserves those helper tokens in printed metadata while `resource get --show-metadata` resolves them against the active payload descriptor, exact payload-type placeholders resolve during template/remote workflows without forcing secret resolution, unknown media types normalize to `application/octet-stream`, and remote request defaults derive `Accept` / `ContentType` from the resolved payload descriptor when metadata leaves them unset.

### Repository Payload
68. Mixed payload repository contract: repository-backed read/write/list/delete flows resolve `resource.*` files by discovered suffix or concrete metadata `resource.format`, reject duplicate payload files for one logical path, read `metadata.yaml` or `metadata.json` sidecars with YAML precedence, merge configured shared metadata with repo-local overlays using repo-local precedence, write repo-local `metadata.yaml` sidecars by default while removing superseded sibling sidecars, preserve opaque input-file suffixes such as `.key` for octet-stream saves, and fall back to `.bin` only when no suffix or stronger payload hint is available.

### Binary Payload
69. Binary payload contract: `application/octet-stream` request and response bodies use `resource.BinaryValue`, `--output auto|text` writes raw bytes for one binary payload without a trailing newline, structured output wraps binary content as base64 with media metadata, multi-item binary auto/text output is rejected, and `resource edit` rejects binary payloads.
70. Binary metadata-mutation contract: body-bearing metadata `transforms` steps run only for structured payloads, while raw text or octet-stream request bodies remain unmodified after metadata rendering.

### E2E Component Contracts
71. E2E component contract validation mode: `run-e2e.sh --validate-components` validates component manifest contract versioning, hook-script syntax, dependency catalog consistency, and managed-service fixture identity metadata (`resource.id` / `resource.alias`) before workload execution, while fast Bash harness tests validate hook semantic contracts for state publication, deterministic context fragments, and repeated hook execution.
72. Plain-text-only CLI commands (`secret get`, `repository tree`, `server get *`, `server check`, shell `completion` subcommands, and `context print-template`) reject `--output json|yaml` with `ValidationError` instead of silently ignoring structured output requests.
...
81. E2E metadata source contract: `run-e2e.sh` defaults `--metadata-source` to `bundle`, accepts only `bundle|dir`, skips component-local `openapi.yaml` wiring and uses shorthand `metadata.bundle` when mapped in `bundle` mode, and uses a run-scoped copy of the component-local `metadata/` tree as `metadata.baseDir` while keeping local OpenAPI wiring in `dir` mode.
...
86. E2E runner selection flag contract: runner selection flags use canonical namespaces for managed-service and shared proxy behavior (`--managed-service`, `--managed-service-connection`, `--managed-service-auth-type`, `--managed-service-mtls`, `--proxy-mode`, `--proxy-auth-type`); prompt-backed generated contexts MUST use top-level `credentials` plus `credentialsRef` instead of deprecated prompt auth blocks.
87. E2E shared proxy contract: `--proxy-mode <none|local|external>` drives explicit proxy injection for CLI contexts (`managedService.http.proxy`, `repository.git.remote.proxy` for `http|https` remotes, `secretStore.vault.proxy`, and `metadata.proxy` for bundle-backed downloads); `local` auto-selects helper component `proxy:forward-proxy`; `external` requires at least one configured proxy URL (`DECLAREST_E2E_PROXY_HTTP_URL` or `DECLAREST_E2E_PROXY_HTTPS_URL`); prompt-backed proxy selections MUST emit top-level credentials with per-attribute `prompt: true` / `persistInSession: true` objects plus `*.proxy.auth.basic.credentialsRef`, MUST keep plaintext proxy credentials out of generated contexts, MUST print generated proxy test credentials in manual handoff output, MUST keep proxy auth bootstrap vars out of the generated setup script, and MUST rewrite compose-local loopback component URLs to a host-reachable non-loopback address when available so host-side CLI traffic actually traverses the proxy.

### OLM Packaging
- OLM bundle/catalog contract: `operator-sdk bundle validate --select-optional suite=operatorframework ./bundle` MUST pass; `bundle/manifests/` MUST include the CSV, owned CRDs, Deployment, RBAC, Services, and PDB; `bundle/metadata/annotations.yaml` MUST declare the `declarest-operator` package on the `alpha` channel; the bundled Deployment MUST use `emptyDir` for the operator state volume; and `opm validate ./catalog` MUST pass for the file-based catalog under `catalog/declarest-operator/`.
- OLM install reference contract: `config/olm/` MUST build via `kustomize build` and install OperatorGroup/CatalogSource/Subscription against the published catalog image, and `make olm-install`/`make olm-uninstall` MUST apply/remove the overlay deterministically.

### Throttling and Operator
88. Managed-service request throttling contract: configured `max-concurrent-requests` and `queue-size` enforce bounded request admission, overflow returns typed `ConflictError`, shared throttling scopes apply across concurrent clients for one managed-service identity, and proxy environment variables merge with configured managed-service proxy fields unless an explicit empty proxy block disables them.
89. SyncPolicy overlap contract: multiple SyncPolicies MAY share repository/managed-service/secret-store references, but overlapping source path/subpath scopes MUST fail deterministically even when references differ.
...
93. Operator reconcile planning contract: sync planning deterministically selects `full` vs `incremental`, metadata/unknown-path diffs trigger safe fallback to full scope, metadata-owned defaults artifact diffs under `/_/defaults.<ext>`, `/_/defaults-<profile>.<ext>`, `<resource>/defaults.<ext>`, or `<resource>/defaults-<profile>.<ext>` resolve to the owning collection or resource apply target, unsupported `defaults*` path diffs trigger safe full fallback, and secret-version-hash changes force reconcile even when repository revision is unchanged.
...
4. CLI test verifies `resource metadata render` fallback from defaulted `get` to `list` when `get` has no path and validates completion scripts do not emit duplicate `--flag`/`--flag=` suggestions.
5. CLI test verifies `resource metadata infer --apply` persists the same compact payload printed to output and that `resource metadata get` default full-shape output vs `--overrides-only` compact output (including endpoint-gated fallback inference) are preserved.
...
8. Bundle metadata tests verify OpenAPI fallback discovery order when `managedService.http.openapi` is empty: `declarest.openapi` hint, bundle-root/metadata-root OpenAPI files, then deterministic recursive bundle-file fallback.
...
10. Unit and harness tests verify metadata sidecars support `metadata.yaml` and `metadata.json`, prefer YAML when both exist, and surface selected managed-service access details in manual-profile handoff output even without a component `manual-info` hook.
...
13. Bash harness tests verify `run-e2e-parallel.sh` returns success only when all child commands succeed, returns failure when any child command fails, and releases run-scoped host-port reservations during finalize or cleanup.
13. Unit + integration tests verify managed-service requestThrottling validation, shared-gate queue overflow behavior, and repository webhook authentication/annotation patch behavior for valid vs invalid payloads.
...
17. Repository, orchestrator, CLI, and operator tests verify metadata defaults merge/compaction semantics, supported defaults codec coverage (`json|yaml|yml|properties`), defaults mode/profile resolution, include expansion validation, local edit/copy/save preservation of compact overrides, defaults CLI flows (`get|edit|config|profile|infer --save|infer --check`) including codec selection and stable empty-object inference, `resource get|save --prune-defaults` behavior, managed-service probe cleanup, and incremental plan classification for add/modify/delete of metadata defaults artifacts.
18. OLM packaging tests verify `kustomize build config/manifests`, `kustomize build config/olm`, `operator-sdk bundle validate --select-optional suite=operatorframework ./bundle`, `opm validate ./catalog`, and release-version consistency checks all succeed; the bundled CSV lists every `declarest.io/v1alpha1` owned CRD with `alm-examples`, declares webhook definitions for every admission webhook, and the bundled Deployment does not carry an OLM-incompatible `PersistentVolumeClaim`.
19. E2E harness tests verify operator profiles apply the vendored OLM core YAML manifests instead of `operator-sdk olm install`, keep OLM admission webhooks enabled in the patched CSV, wait for the run-scoped `CatalogSource` before applying the `Subscription`, wait for the CSV and Deployment before generated DeclaREST CRs, and clean run-scoped OLM objects from reused clusters.
20. CI workflow tests verify `golangci-lint run` through the pinned official action, fast e2e harness contract tests, e2e component validation, GoReleaser config validation, workflow hardening policy (full-SHA actions, no tag publish from manual image smoke workflows, and release attestation/SBOM markers), and path-filtered `make verify-bundle` for OLM-relevant changes.
