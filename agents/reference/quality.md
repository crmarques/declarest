...
55. Plain-text-only CLI commands (`secret get`, `resource-server get *`, `resource-server check`, shell `completion` subcommands, and `config print-template`) reject `--output json|yaml` with `ValidationError` instead of silently ignoring structured output requests.
