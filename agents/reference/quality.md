...
15. Repository defaults-sidecar semantics: `defaults.<ext>` merge/compaction rules, raw defaults-sidecar read/write capability, supported merge-capable payload types (`json|yaml|ini|properties`), unsupported payload rejection (`xml|hcl|text|octet-stream|opaque suffixes`), delete cleanup, and deterministic behavior when only `defaults.<ext>` exists for a logical resource.
...
31. Repository defaults-sidecar contract: repository-backed `resource get|apply|create|update|diff|template|edit|copy|save` use the effective payload merged from `defaults.<ext>` and `resource.<ext>` for merge-capable object resources, while repository writes compact back into `resource.<ext>` so unchanged defaulted fields remain only in `defaults.<ext>`.
32. Resource defaults CLI contract: `resource defaults get|edit|infer` command-level coverage MUST verify raw defaults reads, empty-object behavior when the sidecar is absent, editor persistence/clear behavior, `--save` persistence, `--check` success/mismatch behavior, and the `--managed-server` safety gate requiring `--yes`.
33. Defaults pruning contract: `resource get --prune-defaults` MUST compact repository and managed-server outputs against raw repository defaults sidecars while preserving empty-object output when all fields are defaulted, and `resource save --prune-defaults` MUST compact single-resource and per-item list saves before persistence.
...
93. Operator reconcile planning contract: sync planning deterministically selects `full` vs `incremental`, metadata/unknown-path diffs trigger safe fallback to full scope, `defaults.<ext>` diffs resolve to the parent logical resource target, and secret-version-hash changes force reconcile even when repository revision is unchanged.
...
17. Repository, orchestrator, CLI, and operator tests verify `defaults.<ext>` merge/compaction semantics, supported codec coverage (`json|yaml|ini|properties`), unsupported payload-type rejection, local edit/copy/save preservation of compact overrides, raw defaults CLI flows (`get|edit|infer --save|infer --check`), `resource get|save --prune-defaults` behavior, managed-server probe cleanup, and incremental plan classification for add/modify/delete of defaults sidecars.
