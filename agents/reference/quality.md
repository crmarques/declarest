...
86. E2E managed-server flag contract: runner selection flags use the `--managed-server*` namespace (`--managed-server`, `--managed-server-connection`, `--managed-server-auth-type`, `--managed-server-mtls`, `--managed-server-proxy`, `--managed-server-proxy-auth-type`), `--managed-server-auth-type prompt` emits `managedServer.http.auth.prompt` only for basic-auth-capable components, and legacy `--managed-server*` variants remain rejected.
87. E2E managed-server proxy contract: `--managed-server-proxy` toggles context proxy injection (`managedServer.http.proxy`), `--managed-server-proxy-auth-type prompt` emits `managedServer.http.proxy.auth.prompt`, validation fails when proxying is enabled without at least one configured proxy URL (`DECLAREST_E2E_MANAGED_SERVER_PROXY_HTTP_URL` or `DECLAREST_E2E_MANAGED_SERVER_PROXY_HTTPS_URL`), and prompt/basic proxy auth selections include negative coverage for conflicting or incomplete inline proxy credential inputs.
