...
50. Resource get explicit collection-marker contract: `resource get <path>/` with remote source resolves `<path>` as a collection list target directly (without single-resource remote get), preserving normalized-path semantics and list-output determinism.
