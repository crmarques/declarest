...
### Context and Config
9. Context config: strict decode after legacy-alias normalization, one-of validation (including repository-only and remote-only contexts), metadata-source one-of behavior, shared-source plus repo-local overlay wiring, overrides precedence, and missing-catalog behavior.
...
81. E2E metadata source contract: `run-e2e.sh` defaults `--metadata-source` to `bundle`, accepts only `bundle|dir`, skips component-local `openapi.yaml` wiring and uses shorthand `metadata.bundle` when mapped in `bundle` mode, and uses a run-scoped copy of the component-local `metadata/` tree as `metadata.base-dir` while keeping local OpenAPI wiring in `dir` mode.
...
86. E2E runner selection flag contract: runner selection flags use canonical namespaces for managed-server and shared proxy behavior (`--managed-server`, `--managed-server-connection`, `--managed-server-auth-type`, `--managed-server-mtls`, `--proxy-mode`, `--proxy-auth-type`); `--managed-server-auth-type prompt` emits `managedServer.http.auth.prompt` only for basic-auth-capable components.
87. E2E shared proxy contract: `--proxy-mode <none|local|external>` drives explicit proxy injection for CLI contexts (`managedServer.http.proxy`, `repository.git.remote.proxy` for `http|https` remotes, `secretStore.vault.proxy`, and `metadata.proxy` for bundle-backed downloads); `local` auto-selects helper component `proxy:forward-proxy`; `external` requires at least one configured proxy URL (`DECLAREST_E2E_PROXY_HTTP_URL` or `DECLAREST_E2E_PROXY_HTTPS_URL`); `--proxy-auth-type prompt` emits prompt-backed proxy auth blocks only for `cli-manual`, sets `keepCredentialsForSession: true` for local prompt-backed proxy blocks, prints generated proxy test credentials in manual handoff output, keeps proxy auth bootstrap vars out of the generated setup script, and rewrites compose-local loopback component URLs to a host-reachable non-loopback address when available so host-side CLI traffic actually traverses the proxy; and prompt/basic proxy auth selections include negative coverage for conflicting or incomplete inline proxy credential inputs.
