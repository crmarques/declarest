...
32. Resource defaults CLI contract: `resource defaults get|edit|config get|config edit|profile get|profile edit|profile delete|infer` command-level coverage MUST verify raw metadata persistence vs resolved-defaults output, empty-object behavior when defaults are absent, editor persistence/clear behavior, deterministic defaults artifact file selection, collection-path vs concrete-resource-path inference inputs (including equivalent collection paths with and without trailing `/`), `--save` persistence including defaults-artifact codec selection plus writable-target selection for explicit `metadata.baseDir` versus `metadata.bundle|bundleFile`, `--check` success/mismatch behavior, the `--managed-server` safety gate requiring `--yes`, `--wait` validation plus parsing of bare-integer seconds vs explicit durations, and managed-server inference isolation from sibling/current-defaults value selection.
