...
22. Prompt-auth component auth: prompt-backed managed-server, git, vault, and proxy flows prompt once per command when uncached, fail without an interactive terminal when no cached session credentials are available, ask for explicit reuse confirmation before sharing credentials with other prompt-auth components in the same command, and reuse kept component credentials across later commands in the same terminal session when session persistence is available.
