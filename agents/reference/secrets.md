...
18. `resource get` MUST redact values for metadata `resource.secretAttributes` as `{{secret .}}` placeholders by default for repository and managed-service output modes, and MUST redact whole-resource secret scopes to the same placeholder when `resource.secret: true` applies.
