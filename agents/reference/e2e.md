...
34. Runner security selection flags MUST include `--managed-server-auth-type <none|basic|oauth2|custom-header|prompt>` and `--managed-server-mtls`; `--managed-server-mtls` defaults to `false`, auth type defaults MUST be elected by the selected managed-server component when the flag is omitted (preference order SHOULD be `oauth2`, then `custom-header`, then `basic`, then `none`), and `prompt` MUST emit top-level prompt-backed credentials plus `managedServer.http.auth.basic.credentialsRef` for managed-server components that support `basic-auth`.
...
10. `./run-e2e.sh --profile cli-basic --managed-server keycloak` emits a context with `metadata.bundle: keycloak-bundle:0.0.1` (and no `metadata.baseDir`) so keycloak runs consume metadata from the default remote bundle source.
11. `./run-e2e.sh --profile cli-basic --managed-server simple-api-server --metadata-source dir` emits `metadata.baseDir` from `test/e2e/components/managed-server/simple-api-server/metadata` and keeps local `managed-server.http.openapi`.
...
15. `./run-e2e.sh --profile cli-manual --managed-server simple-api-server --managed-server-auth-type prompt` emits top-level prompt-backed credentials plus `managedServer.http.auth.basic.credentialsRef` while the local component state still keeps basic-auth credentials available for manual handoff details.
16. `./run-e2e.sh --profile cli-manual --platform compose --proxy-mode local --proxy-auth-type prompt` injects top-level prompt-backed proxy credentials plus proxy `basic.credentialsRef` placeholders, rewrites local managed-server and other eligible loopback URLs to one host-reachable non-loopback address when available so the proxy is exercised, keeps inline proxy credentials out of `contexts.yaml`, prints the generated proxy credentials in manual handoff output, and leaves proxy auth bootstrap env vars unset in the generated setup script.
...
21. `./run-e2e.sh --profile cli-basic --managed-server rundeck` emits `metadata.baseDir` from `test/e2e/components/managed-server/rundeck/metadata` because `rundeck` has no shorthand bundle mapping, while still leaving local `managed-server.http.openapi` unset.
