...
42. In `bundle` mode, the runner MUST skip local `openapi.yaml` wiring so `managed-service.http.openapi` remains unset, and MUST use the selected managed-service component `METADATA_BUNDLE_REF` when declared. When that ref uses the `oci://` scheme, the runner MUST skip local cache seeding and let the declarest resolver pull the OCI artifact from the advertised registry at first use; when that ref uses any other form (shorthand, URL, or local path), the runner MAY pre-seed the local cache from the peer bundles repo at `E2E_METADATA_BUNDLES_ROOT` for offline/deterministic runs.
...
13. `managed-service=keycloak` runs in `bundle` mode fail during context validation when the default OCI ref `oci://ghcr.io/crmarques/declarest-metadata-bundles/keycloak:0.0.1` cannot be pulled from the registry.
...
10. `./run-e2e.sh --profile cli-basic --managed-service keycloak` emits a context with `metadata.bundle: oci://ghcr.io/crmarques/declarest-metadata-bundles/keycloak:0.0.1` (and no `metadata.baseDir`) so keycloak runs pull metadata from the published OCI artifact on GHCR on first use.
...
21. `./run-e2e.sh --profile cli-basic --managed-service rundeck` emits `metadata.bundle: oci://ghcr.io/crmarques/declarest-metadata-bundles/rundeck:0.0.1` and resolves it via the OCI registry at first CLI invocation, while still leaving local `managed-service.http.openapi` unset; setting `--metadata-source dir` makes the same run fall back to the peer `declarest-metadata-bundles/bundles/rundeck/` tree (overridable via `E2E_METADATA_BUNDLES_ROOT`).
...
31. `./run-e2e.sh --profile cli-basic --managed-service haproxy` emits `metadata.bundle: oci://ghcr.io/crmarques/declarest-metadata-bundles/haproxy:0.0.1` and resolves it via the OCI registry at first CLI invocation; `--metadata-source dir` still falls back to the peer `declarest-metadata-bundles/bundles/haproxy/` tree so local/filesystem coverage remains exercised. Local `managed-service.http.openapi` remains unset in both modes.
