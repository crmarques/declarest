...
21. `./run-e2e.sh --profile cli-basic --managed-service rundeck` emits `metadata.bundle: rundeck:0.1.0` resolved from the peer `declarest-metadata-bundles/bundles/rundeck/` tree (overridable via `E2E_METADATA_BUNDLES_ROOT`), while still leaving local `managed-service.http.openapi` unset.
...
31. `./run-e2e.sh --profile cli-basic --managed-service haproxy` emits `metadata.bundle: haproxy:0.1.0` resolved from the peer `declarest-metadata-bundles/bundles/haproxy/` tree (overridable via `E2E_METADATA_BUNDLES_ROOT`) and leaves local `managed-service.http.openapi` unset.
