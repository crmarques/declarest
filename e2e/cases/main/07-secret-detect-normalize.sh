...
case_run() {
  ...
  case_run_declarest resource save "${repo_detect_scope}/acme" --insecure -f "${repo_detect_acme_payload_file}" -i json
  case_expect_success
  case_run_declarest resource save "${repo_detect_scope}/beta" --insecure -f "${repo_detect_beta_payload_file}" -i json
  case_expect_success
