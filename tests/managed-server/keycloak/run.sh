...
usage() {
    cat <<EOF
Usage: ./tests/managed-server/keycloak/run.sh [setup|sync|stop|clean|reset|cli] [options] [-- <declarest args>]
    ...
    setup)
        ...
        if [[ "$repo_type" == "git-remote" ]]; then
            ...
            if [[ -z "${DECLAREST_REPO_REMOTE_URL:-}" ]]; then
                case "$provider" in
                    gitlab)
                        run_step "Preparing GitLab" "$PROVIDER_SCRIPTS_DIR/gitlab/setup.sh"
                        provider_env="$DECLAREST_WORK_DIR/gitlab.env"
                        ;;
                    gitea)
                        run_step "Preparing Gitea" "$PROVIDER_SCRIPTS_DIR/gitea/setup.sh"
        ...
        write_state
        run_step "Preparing template repo" "$REPO_SCRIPTS_DIR/prepare.sh"
        ...
        if [[ "$secret_store_type" == "none" && "$repo_type" == "git-remote" ]]; then
            run_step "Sanitizing repository (no secrets)" "$REPO_SCRIPTS_DIR/strip-secrets.sh" "$DECLAREST_REPO_DIR"
        fi
        if [[ "$secret_store_type" != "none" ]]; then
...
Stop Keycloak:
  ./tests/managed-server/keycloak/run.sh stop --work-dir "${DECLAREST_WORK_DIR}"
