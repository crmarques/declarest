...
func extractListItems(payload any) ([]any, error) {
	...
	case map[string]any:
		...
		if ok {
			...
			if !valuesOK {
				return nil, managedservicedomain.NewListPayloadShapeError("list response \"items\" must be an array", nil)
			}
			return values, nil
		}
		...
		if len(arrayFieldKeys) > 1 {
			return nil, managedservicedomain.NewListPayloadShapeError(
				fmt.Sprintf(
		...
		return nil, managedservicedomain.NewListPayloadShapeError("list response object must include an \"items\" array", nil)
	default:
		return nil, managedservicedomain.NewListPayloadShapeError("list response must be an array or an object with an \"items\" array", nil)
	}
}
...
func (g *Client) decodeListResponse(
	...
	for _, item := range items {
		...
		if !ok {
			return nil, managedservicedomain.NewListPayloadShapeError("list payload entries must be JSON objects", nil)
		}
		...
		if !ok {
			return nil, managedservicedomain.NewListPayloadShapeError("list payload entry normalization failed", nil)
		}
