...
func (r *GitResourceRepository) computeAheadBehind(repo *gogit.Repository, localHash plumbing.Hash, remoteHash plumbing.Hash) (int, int, error) {
	localSet, err := r.collectCommitSet(repo, localHash)
	if err != nil {
		return 0, 0, err
	}
	remoteSet, err := r.collectCommitSet(repo, remoteHash)
	if err != nil {
		return 0, 0, err
	}

	ahead := countSetDifference(localSet, remoteSet)
	behind := countSetDifference(remoteSet, localSet)
