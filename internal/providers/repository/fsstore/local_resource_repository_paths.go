...
func (r *LocalResourceRepository) canonicalPayloadFilePath(logicalPath string, payloadType string) (string, error) {
	...
	if !fsutil.IsPathUnderRoot(r.baseDir, filePath) {
		return "", faults.Invalid("logical path escapes repository base directory", nil)
	}
	return filePath, nil
}
...
func normalizeArtifactRelativePath(file string) (string, error) {
	...
	if trimmed == "" {
		return "", faults.Invalid("resource artifact file must not be empty", nil)
	}
	if strings.HasPrefix(trimmed, "/") {
		return "", faults.Invalid("resource artifact file must stay within the resource directory", nil)
	}
