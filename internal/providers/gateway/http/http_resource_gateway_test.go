...
func TestNewHTTPResourceGatewayValidation(t *testing.T) {
	...
	t.Run("missing_base_url", func(t *testing.T) {
		...
		_, err := NewHTTPResourceGateway(config.HTTPServer{
...
func TestAuthModesAndOAuth2Caching(t *testing.T) {
	...
	t.Run("oauth2_client_credentials_cached", func(t *testing.T) {
		...
		if _, err := gateway.Get(context.Background(), resourceInfo, md); err != nil {
	...
	t.Run("oauth2_debug_logs_include_token_request", func(t *testing.T) {
		...
		if _, err := gateway.Get(ctx, resourceInfo, md); err != nil {
			t.Fatalf("Get returned error: %v", err)
		}
...
func TestListResponseShapesAndAliasRules(t *testing.T) {
	...
	t.Run("list_operation_jq_resource_function_uses_context_resolver", func(t *testing.T) {
		...
		ctx := gatewaydomain.WithListJQResourceResolver(
			context.Background(),
...
func mustGateway(t *testing.T, cfg config.HTTPServer) *HTTPResourceGateway {
	t.Helper()

	gateway, err := NewHTTPResourceGateway(cfg)
	if err != nil {
		t.Fatalf("NewHTTPResourceGateway returned error: %v", err)
	}
	return gateway
}
