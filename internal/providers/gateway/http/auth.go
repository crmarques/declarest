...
func (g *HTTPResourceGateway) applyAuth(ctx context.Context, request *http.Request) error {
	switch g.auth.mode {
	case authModeOAuth2:
...
func (g *HTTPResourceGateway) oauthToken(ctx context.Context) (string, error) {
	g.oauthMu.Lock()
