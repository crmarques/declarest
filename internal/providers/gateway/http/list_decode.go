...
func extractListItems(payload any) ([]any, error) {
	...
	case map[string]any:
		...
		if ok {
			...
			if !valuesOK {
				return nil, gatewaydomain.NewListPayloadShapeError("list response \"items\" must be an array", nil)
			}
			return values, nil
		}
		...
		if len(arrayFieldKeys) > 1 {
			return nil, gatewaydomain.NewListPayloadShapeError(
				fmt.Sprintf(
		...
		return nil, gatewaydomain.NewListPayloadShapeError("list response object must include an \"items\" array", nil)
	default:
		return nil, gatewaydomain.NewListPayloadShapeError("list response must be an array or an object with an \"items\" array", nil)
	}
}
...
func (g *HTTPResourceGateway) decodeListResponse(
	ctx context.Context,
	...
	for _, item := range items {
		...
		if !ok {
			return nil, gatewaydomain.NewListPayloadShapeError("list payload entries must be JSON objects", nil)
		}
		...
		if !ok {
			return nil, gatewaydomain.NewListPayloadShapeError("list payload entry normalization failed", nil)
		}
