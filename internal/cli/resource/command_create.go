...
func newCreateCommand(deps cliutil.CommandDependencies, globalFlags *cliutil.GlobalFlags) *cobra.Command {
	var pathFlag string
	...
	command := &cobra.Command{
		...
		Long: strings.Join([]string{
			...
			"Use --refresh to fetch the remote state after the create and persist it locally.",
		}, " "),
		Example: strings.Join([]string{
			...
			"  declarest resource create /customers/acme --refresh",
		}, "\n"),
		...
		RunE: func(command *cobra.Command, args []string) error {
			...
			result, err := mutateapp.Execute(runCtx, mutateapp.Dependencies{
				...
				Operation:        mutateapp.OperationCreate,
