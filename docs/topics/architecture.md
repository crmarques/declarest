...
- **OLM bundle and catalog**: operator-sdk `registry+v1` bundle under `bundle/` plus a file-based catalog under `catalog/declarest-operator/`. Release automation regenerates both from the same `VERSION`, validates that the CSV and catalog reference matching images, publishes `ghcr.io/crmarques/declarest-operator-bundle:<VERSION>` and `ghcr.io/crmarques/declarest-operator-catalog:<VERSION>`, and consumes the catalog through the reference `install-olm.yaml` (`OperatorGroup`, `CatalogSource`, `Subscription`). Uses `emptyDir` for operator state because OLM's `registry+v1` format forbids bundled PVCs.
